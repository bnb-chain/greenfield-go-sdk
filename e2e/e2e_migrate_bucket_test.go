...
//	txhash, err := s.Client.MigrateBucket(s.ClientContext, bucketName, types.MigrateBucketOptions{TxOpts: nil, DstPrimarySPID: 2, IsAsyncMode: false})
//	s.Require().NoError(err)
//
...
//	bucketInfo, err = s.Client.HeadBucket(s.ClientContext, bucketName)
//	s.Require().NoError(err)
//	s.Require().Equal(bucketInfo.BucketStatus, storageTypes.BUCKET_STATUS_MIGRATING)
//
