...
type ObjectAndBucketIDs struct {
	// IDs defines the id of objects and buckets
	IDs []uint64 `xml:"IDs"`
}
...
type PaymentAccounts struct {
	// PaymentAccount defines the payment account info
	PaymentAccount *PaymentAccount `xml:"PaymentAccount"`
	// StreamRecord defines stream payment records of a stream account
...
type ListObjectPoliciesResponse struct {
	// Policies define a list of policy meta
